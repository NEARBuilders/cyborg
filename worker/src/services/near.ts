/**
 * NEAR Service for Cloudflare Workers
 *
 * Handles NEAR blockchain interactions for NFT rank detection.
 * Adapted from api/src/services/near.ts with Workers-compatible APIs.
 *
 * Key changes from Node.js version:
 * - Buffer.from() → btoa() for base64 encoding
...
export class NearService {
  ...
  async hasInitiateToken(nearAccountId: string): Promise<boolean> {
    try {
      const tokens = await this.fetchNftsFromChain(
        nearAccountId,
  ...
  async getUserRank(nearAccountId: string): Promise<RankData | null> {
    ...
    try {
      ...
      if (rankData) {
        ...
        console.log(
          `[NearService] Found ${rankData.rank} rank for ${nearAccountId} (token: ${rankData.tokenId})`
        );
      }
   ...
   * Uses btoa() instead of Buffer.from() for Workers compatibility
   */
  private async fetchNftsFromChain(
    ...
    try {
      // Prepare RPC request - use btoa() for base64 encoding (Workers-compatible)
