...
**ASCENDANT RANK LEGIONNAIRE** - You have earned your first skillcape! You can receive helpful responses (up to 1200 tokens).`;
  ...
  private async listLegionMembers(params: {
    ...
    try {
      ...
      for (const builder of builders) {
        markdown += `### **${builder.roleEmoji} ${builder.displayName}** | [View Profile](/profile/${builder.accountId})\n`;
        markdown += `**@${builder.accountId}** • **${builder.role}**\n\n`;
        markdown += `![${builder.displayName}](${builder.avatar})\n\n`;

        if (builder.description) {
          markdown += `${builder.description}\n\n`;
        }
  ...
  private async getNftHolders(params: { contractId: string; limit?: number }): Promise<string> {
    ...
    try {
      ...
      // Fetch profile data for each holder
      ...
      return JSON.stringify({
        ...
        count: holdersWithProfiles.length,
        data: holdersWithProfiles,
      });
    } catch (error) {
