...
export default defineConfig({
  ...
  source: {
    define: {
      ...
      "process.env.USE_REMOTE_API": JSON.stringify(
        process.env.USE_REMOTE_API || "false"
      ),
      "process.env.USE_REMOTE_UI": JSON.stringify(
        process.env.USE_REMOTE_UI || "false"
      ),
      "process.env.PUBLIC_ACCOUNT_ID": JSON.stringify(bosConfig.account),
      "process.env.BETTER_AUTH_URL": JSON.stringify(
        process.env.BETTER_AUTH_URL ||
          bosConfig.app.host[process.env.NODE_ENV || "development"]
      ),
    },
    entry: {
