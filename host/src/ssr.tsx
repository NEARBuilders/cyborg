...
function extractHeadData(router: AnyRouter): HeadData {
  const meta: HeadMeta[] = [];
...
export function createSSRHtml(
  ...
  const serializedConfig = safeJsonStringify(clientConfig);
  const serializedState = safeJsonStringify(dehydratedState);
...
${tags}
  ...
  <script>window.__RUNTIME_CONFIG__=${serializedConfig};</script>
